...
export default function Produtos() {
  ...
  async function carregarProdutos() {
    ...
    lista.sort((a, b) => {
      const dataA = a.created_at?.toDate
        ? a.created_at.toDate()
        : new Date(a.created_at || 0);

      const dataB = b.created_at?.toDate
        ? b.created_at.toDate()
  ...
  function editarProduto(produtoSelecionado) {
    ...
    setProduto({
      ...
      highlight_1:
        produtoSelecionado.highlight_1 || "Feito sob encomenda",

      highlight_2:
        produtoSelecionado.highlight_2 || "Personalização completa",

      highlight_3:
        produtoSelecionado.highlight_3 || "Produção artesanal",

      highlight_4:
        produtoSelecionado.highlight_4 || "Atendimento pelo WhatsApp",

      has_variations: produtoSelecionado.has_variations ?? false,

      variation_label:
        produtoSelecionado.variation_label || "Tamanho",
  ...
  async function copiarTexto(texto) {
    ...
    try {
      ...
      alert(
        "Copie manualmente pelo campo. Sim, tecnologia às vezes só decora o ambiente."
      );
    }
  }
  ...
  function atualizarVariacao(index, campo, valor) {
    setProduto((prev) => {
      const lista = Array.isArray(prev.variations)
        ? [...prev.variations]
        : [];
  ...
  function removerVariacao(index) {
    setProduto((prev) => {
      ...
      return {
        ...
        variations: lista,
        has_variations:
          lista.length > 0 ? prev.has_variations : false,
      };
    });
  }
  ...
  async function salvarProduto(e) {
    ...
    try {
      const dadosProduto = {
        ...
        variations: produto.has_variations
          ? variacoesLimpas
          : [],
  ...
  const totalComVariacoes =
    produtos.filter(
      (item) => item.has_variations
    ).length;

  function renderCampoUrl(
    campo,
    label,
    tipo = "imagem"
  ) {
        ...
        <div style={linhaEntreItens}>
          ...
          >
            ...
            <button
              type="button"
              onClick={() =>
                abrirUrlExterna(valor)
              }
              style={botaoPequenoRosa}
            >
              Abrir
            </button>
          </div>
        ...
        <input
          ...
          placeholder={
            tipo === "video"
              ...
              : `Cole a URL da ${label.toLowerCase()}`
          }
        />

        {valor ? (
          tipo === "video" ? (
            <div style={previewVazio}>
              🎬 Vídeo cadastrado.
              Use “Abrir” para conferir.
            </div>
          ) : (
            <div
              style={previewImagemBox}
            >
              <img
                src={valor}
                alt={`Prévia ${label}`}
          ...
          <div style={previewVazio}>
            {tipo === "video"
              ? "🎬 Cole uma URL de vídeo."
              : "🖼️ Cole uma URL para ver a prévia."}
          </div>
        )}
      </div>
    );
    ...
    <section>
      <div style={paginaCabecalho}>
        <div>
          ...
          <p style={subtitulo}>
            Gerencie seu catálogo com
            busca, filtros e cadastro em
            modal. Civilização,
            finalmente.
          </p>
        </div>
          ...
          >
            ...
            <div style={modalBody}>
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ex: Capa dura e Wire-O"
                  />
                </div>
              </div>
              ...
              <textarea
                ...
                placeholder="Ex: Nome, logo, cores, tema e elementos personalizados"
              />
...
const botaoPrincipalTopo = {
  ...
  fontWeight: 800,
  boxShadow:
    "0 10px 25px rgba(236,25,113,.20)",
};
